...
Suites of digits are considered as an instance of the Integer class :\\
eval(n) = eval(Integer$<$-\{new,n\}) = $<$Integer n$>$ Where n $\in \mathbb\{Z\}$\\
...
Ex: eval($<$Integer 4$>$ + $<$Integer 2$>$) = eval($<$Integer 4$>$ $<$- \{add, $<$Integer 2$>$\}) \\= eval(Integer $<$- \{new 6\}) = $<$Integer 6$>$\\
The second part (leq, geq, eq, neq, lt, gt), return a new Boolean that also has as value the result of the operation\\
Ex: eval($<$Integer 4$>$ $>$ $<$Integer 2$>$) =  eval($<$Integer 4$>$ $<$- \{lt, $<$Integer 2$>$\}) \\ = eval(Boolean $<$- \{new true\}) = $<$Boolean true$>$
